...
AUDIO_FILE_PATH = "your_audio_file.mp3"  # Path to the audio file to be played
...
# Track the last command usage time for rate limiting
last_used = {}
...
# Command to start playing audio
@bot.on_message(filters.command("play"))
async def play_audio(client, message: Message):
    if not is_authorized(message.from_user.id):
        logger.warning(f"Unauthorized play attempt by user {message.from_user.id} at {datetime.now()}")
    ...
    try:
        logger.info(f"User {message.from_user.id} is attempting to play audio at {datetime.now()}")
        
        # Play the audio in the group call
        await tg_calls.join_group_call(message.chat.id, AUDIO_FILE_PATH)
        logger.info(f"User {message.from_user.id} successfully started playing audio at {datetime.now()}")
        await message.reply("Audio is now playing in the call!")
    except Exception as e:
        logger.error(f"Error occurred for user {message.from_user.id} while playing audio: {e}")
        await message.reply("Failed to play audio. Please try again later.")
