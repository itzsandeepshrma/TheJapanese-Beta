...
# Command to stop the group call
@bot.on_message(filters.command("stop"))
async def stop_vc(client, message: Message):
    if not is_authorized(message.from_user.id):
        logger.warning(f"Unauthorized stop attempt by user {message.from_user.id} at {datetime.now()}")
        await message.reply("You are not authorized to stop the call!")
        return
    ...
    try:
        logger.info(f"User {message.from_user.id} is attempting to stop the group call at {datetime.now()}")
        
        # Stop the group call
        await tg_calls.leave_group_call(message.chat.id)
        logger.info(f"User {message.from_user.id} successfully stopped the group call at {datetime.now()}")
        await message.reply("Group call has been stopped successfully.")
    except Exception as e:
        logger.error(f"Error occurred for user {message.from_user.id} while stopping the call: {e}")
        await message.reply("Failed to stop the call. Please try again later.")
